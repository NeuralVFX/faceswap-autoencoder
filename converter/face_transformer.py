...
class FaceTransformer(object):
    ...
    def _preprocess_inp_img(self, inp_img, roi_coverage, IMAGE_SHAPE):
        ...
        roi = img_bgr[roi_x:-roi_x, roi_y:-roi_y,:] 
        roi_size = roi.shape
        ae_input = cv2.resize(roi, IMAGE_SHAPE[:2])/255. * 2 - 1 
    ...
    def _ae_forward_pass(self, ae_input):
        ae_input = np.rollaxis(ae_input, 2)
        ae_input = torch.FloatTensor(ae_input)
        ae_input = ae_input.cuda()

        ae_out = self.path_func(ae_input.unsqueeze(0))
        ae_out = ae_out.cpu()
        ae_out = ae_out.data
        ae_out = ae_out.numpy()[0].transpose(1, 2, 0)
        self.ae_output = ae_out      
    ...
    def transform(self, inp_img, direction, roi_coverage, color_correction, IMAGE_SHAPE):
        ...
        self.model.model_dict["DEC_A"].eval()
        self.model.model_dict["DEC_B"].eval()
        if direction == "AtoB":
            self.path_func = nn.Sequential(self.model.model_dict["ENC"],self.model.model_dict["DEC_B"]).eval()
        elif direction == "BtoA":
            self.path_func = nn.Sequential(self.model.model_dict["ENC"],self.model.model_dict["DEC_A"]).eval()
